window.onload = function () {
    ...
    const SIZE = 60;
    const SPRITES = {
        head_down: [256, 64], head_up: [192, 0], head_left: [192, 64], head_right: [256, 0], apple: [0, 192], body_horizontal: [64, 0],
    ...
    //                                         [x,y] 
    let snake = { head: [5, Math.round((N - 1) / 2)], turns: [[2, Math.round((N - 1) / 2)]], head_position: SPRITES.head_right, animationDirection: 1, moves: [], length: 4, turnfacing: null, facing: "ArrowRight", turning: false, growing: false, animationShift: -SIZE, last_tail: [2, Math.round((N - 1) / 2)] }
    ...
    function draw_snake_and_apple() {
        ...
        snake.turns.forEach(turn => {
            ...
            if (snakePieces === snake.length) {
                if (snake.last_tail[1] == turn[1]) {
                    let tail_direction = (snake.last_tail[0] - turn[0]) > 0 ? 1 : -1
                    let canvasX = current_position[0] * SIZE + snake.animationShift * -tail_direction
                    let canvasY = current_position[1] * SIZE
                    if (tail_direction === -1) ctx.drawImage(spritesheet, SPRITES.tail_left[0], SPRITES.tail_left[1], 64, 64, canvasX, canvasY, SIZE, SIZE)
                    else ctx.drawImage(spritesheet, SPRITES.tail_right[0], SPRITES.tail_right[1], 64, 64, canvasX, canvasY, SIZE, SIZE)
                } else if (snake.last_tail[0] == turn[0]) {
                    let tail_direction = (snake.last_tail[1] - turn[1]) > 0 ? 1 : -1
                    let canvasX = current_position[0] * SIZE
                    let canvasY = current_position[1] * SIZE + snake.animationShift * -tail_direction
                    if (tail_direction === 1) ctx.drawImage(spritesheet, SPRITES.tail_down[0], SPRITES.tail_down[1], 64, 64, canvasX, canvasY, SIZE, SIZE)
        ...
        snake.turns.forEach((turn, index) => {
            let turndir = (snake.turns.length >= 2) ? snake.turns[snake.turns.length - 2] : snake.head
            let num = 1
            if (turndir[0] != snake.last_tail[0] && turndir[1] != snake.last_tail[1]) {
                num = 0
            }
            if (index != snake.turns.length - num) {

                const nextTurn = snake.turns[index + 1] ? snake.turns[index + 1] : snake.last_tail
                ...
                if (turn[0] - nextTurn[0] != 0) {
                    ...
                    if (prevTurn) {
                        ...
                        if (index != snake.turns.length - 1) ctx.drawImage(spritesheet, SPRITES.body_horizontal[0], SPRITES.body_horizontal[1], 64, 64, canvasX, canvasY, SIZE, SIZE)
                    ...
                    if (prevTurn) {
                        ...
                        if (index != snake.turns.length - 1) ctx.drawImage(spritesheet, SPRITES.body_vertical[0], SPRITES.body_vertical[1], 64, 64, canvasX, canvasY, SIZE, SIZE)
                        ctx.clearRect(turn[0] * SIZE + 1, turn[1] * SIZE + 1, SIZE - 2, SIZE - 2)
                        if (direction > 0) {
                            if (prevtDir > 0) ctx.drawImage(spritesheet, SPRITES.turn_up_right[0], SPRITES.turn_up_right[1], 64, 64, turn[0] * SIZE, turn[1] * SIZE, SIZE, SIZE)
                            else ctx.drawImage(spritesheet, SPRITES.turn_up_left[0], SPRITES.turn_up_left[1], 64, 64, turn[0] * SIZE, turn[1] * SIZE, SIZE, SIZE)
